...
public class ThroughputLatencyServer extends DefaultSingleRecoverable {
    private final Logger logger = LoggerFactory.getLogger("sharbft");
    private byte[] state;
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println("USAGE: sharbft.benchmark.ThroughputLatencyServer <process id>");
            System.exit(-1);
        }
    ...
    private void printMeasurement() {
        ...
        System.out.println("sdasdasdadAHHSHASHSHDHAH");
        System.out.println((int) (deltaTime / 2));
        ...
        if (true) {
            ...
            logger.debug("M:(clients[#]|requests[#]|delta[ns]|throughput[ops/s], max[ops/s])>("+senders.size()+"|"+numRequests+"|"+delta+"|"+throughput+"|"+maxThroughput+")");
            numRequests = 0;
